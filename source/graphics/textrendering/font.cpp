...
void Font::BuildVertexArray(void* outVertexArray, const std::string& textToRender, float drawX, float drawY)
{
    ...
    for (uint32_t i = 0; i < textToRenderSize; ++i)
    {
        ...
        else
        {
            ...
            vertexPtr[index].m_Position = vec4(drawX, drawY, 0.0f, 1.f);  // Top left.
            vertexPtr[index].m_UV = vec2(m_Font[letter].left, 0.0f);
            index++;

            vertexPtr[index].m_Position = vec4((drawX + m_Font[letter].size * scaleWidth), (drawY - 16 * scaleHeight), 0.0f, 1.f);  // Bottom right.
            vertexPtr[index].m_UV = vec2(m_Font[letter].right, 1.0f);
            index++;

            vertexPtr[index].m_Position = vec4(drawX, (drawY - 16 * scaleHeight), 0.0f, 1.f);  // Bottom left.
            vertexPtr[index].m_UV = vec2(m_Font[letter].left, 1.0f);
            index++;
            ...
            vertexPtr[index].m_Position = vec4(drawX + m_Font[letter].size * scaleWidth, drawY, 0.0f, 1.f);  // Top right.
            vertexPtr[index].m_UV = vec2(m_Font[letter].right, 0.0f);
            index++;
